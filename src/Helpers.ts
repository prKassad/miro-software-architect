 ...
 * @param widgetId wdiget id
 */
export const findRootWidgetId = async (
  widgetId: string
): Promise<string | null> => {
 ...
 */
export const findChildrenWidgetIds = async (
  widgetId: string
): Promise<string[]> => {
  ...
  //   .filter(s => s !== null);
  return edges
    .map((e: SDK.ILineWidget) => e.endWidgetId || "")
    .filter(s => s !== "");
};
 ...
 */
export const findReachableWidgetIds = async (
  widgetId: string,
  ...
  if (rootId) {
    ...
    return includeCurrent
      ? result
      : result.filter((id: string) => id !== widgetId);
  } else {
