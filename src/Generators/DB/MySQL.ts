import {DataBaseFieldType, IDataBaseModel, IDatabaseGenerator} from '../../Interfaces'
			...
			model.entities
				.map((entity) => {
					const fields = entity.fields.map((field) => {
						const flags: string[] = []
						if (field.defaultValue && field.defaultValue !== '') flags.push(`DEFAULT '${field.defaultValue}'`)
						...
						return `  \`${field.name}\` ${DataBaseFieldType[field.type]}${length}${sep}${flags.join(' ')},`
					})
