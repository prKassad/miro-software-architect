...
import {genEnityFieldWidget, genFieldTextWidget, genServiceEdgeWidget} from '../../../Helpers'
...
export class DatabaseEntityEditor extends Component<IEditorProps, IEditorState> {
	componentDidMount() {
	...
	async updateField(index: number, field: IEntityFieldMetaWithWidgetId): Promise<void> {
		const fields: IEntityFieldMetaWithWidgetId[] = [...this.state.fieldContainers]
		fields[index] = field
		this.setState({
			...this.state,
	...
	async addField(): Promise<void> {
		...
		newfieldContainers.push(Object.assign(widget[0].metadata[APP_ID].data, {widgetId: widget[0].id}))
		...
		await miro.board.selection.selectWidgets([...selectionWidgets, ...[widget[0].id]])
	...
	async removeField(index: number): Promise<void> {
		...
		const fieldContainers: IEntityFieldMetaWithWidgetId[] = [...this.state.fieldContainers].filter(
			(_, ind) => ind !== index,
		)
	...
	async rebuild(firstFieldRemoved: boolean = false, firstFieldCreated: boolean = false) {
		...
		if (!firstFieldCreated && this.state.fieldContainers.length) {
			...
			top = firstFieldRemoved ? firstField.bounds.top - height : firstField.bounds.top
		...
		if (this.state.fieldContainers.length) {
			await miro.board.widgets.update({
				...
				...convertStartToCenterCoords(
					...
					width,
					height * (this.state.fieldContainers.length + EMPTY_CELLS_ON_FIELDS_CONTINER),
				),
			})
			...
			this.state.fieldContainers.forEach(async (field, index) => {
				miro.board.widgets.update({
					id: field.widgetId,
					...convertStartToCenterCoords(left, top + index * height, width, height),
					clientVisible: true,
					metadata: {
						[APP_ID]: {
							widgetType: WidgetType.ENTITY_FIELD_CONTINER,
