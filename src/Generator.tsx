...
async function init() {
	...
	selectedWidgets.forEach(async (widget: SDK.IWidget) => {
		...
		if (widgetAppMetadata && widgetAppMetadata.widgetType === WidgetType.ENTITY_NAME_CONTAINER) {
	...
	entities = await Promise.all(
		entities.map(async (entity) => {
			...
			const widgetsByName = selectedWidgets.filter((widget) => reachableIds.indexOf(widget.id) !== -1)
			const fields = widgetsByName
				.map((widget) => {
					if (widget.metadata[APP_ID] && widget.metadata[APP_ID].widgetType === WidgetType.ENTITY_FIELD_CONTINER) {
