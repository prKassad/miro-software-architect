import {genEnitityNameWidget, genEnityFieldWidget, genEntityFieldsWidget, genServiceEdgeWidget} from './Helpers'
 ...
 * @param name
 */
async function createElement(x: number, y: number, color: string, name: string) {
	...
	for (let i = 1; i < ids.length; i++) {
		serviceEdges.push(genServiceEdgeWidget({startWidgetId: ids[0], endWidgetId: ids[i]}))
	}
